...
tmp=$(curl -s https://gitlab.com/api/v4/projects/${CI_PROJECT_ID}/pipelines\?ref\=develop\&status\=success | jq '.[0] | .id')

# pass that into the curl below
develop=$(curl -s https://gitlab.com/api/v4/projects/${CI_PROJECT_ID}/pipelines/${tmp} | jq '.coverage')
